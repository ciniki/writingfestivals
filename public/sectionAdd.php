...
// -------
//
function ciniki_writingfestivals_sectionAdd(&$ciniki) {
    //
    ...
    $rc = ciniki_writingfestivals_checkAccess($ciniki, $args['tnid'], 'ciniki.writingfestivals.sectionAdd');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( $rc['num_rows'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.writingfestivals.17', 'msg'=>'You already have a section with that name, please choose another.'));
    }
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.writingfestivals.section', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.writingfestivals.section', 'object_id'=>$section_id));
