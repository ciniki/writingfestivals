...
// -------
//
function ciniki_writingfestivals_sectionUpdate(&$ciniki) {
    //
    ...
    $rc = ciniki_writingfestivals_checkAccess($ciniki, $args['tnid'], 'ciniki.writingfestivals.sectionUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( isset($args['name']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.writingfestivals.23', 'msg'=>'You already have an section with this name, please choose another.'));
        }
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.writingfestivals.section', $args['section_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.writingfestivals.section', 'object_id'=>$args['section_id']));
