...
// -------
//
function ciniki_writingfestivals_categoryUpdate(&$ciniki) {
    //
    ...
    $rc = ciniki_writingfestivals_checkAccess($ciniki, $args['tnid'], 'ciniki.writingfestivals.categoryUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( isset($args['name']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.writingfestivals.36', 'msg'=>'You already have an category with this name, please choose another.'));
        }
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.writingfestivals.category', $args['category_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.writingfestivals.category', 'object_id'=>$args['category_id']));
